...
데스크톱 앱에서 문서를 열고 **한국어로 번역**을 누르면 Google 번역으로 본문을 한국어로 표시합니다. API 키, 가입, 결제 등록이 필요하지 않습니다. **원문 보기**로 전환할 수 있고, 같은 문서에서 다시 번역 보기로 전환하면 기존 결과를 사용합니다. 원본 파일은 수정하지 않습니다.

번역 버튼을 누를 때 제목·본문·표·링크의 텍스트가 Google로 전송됩니다. 코드 블록, 인라인 코드, 다이어그램, 링크 주소와 이미지는 유지합니다. 긴 텍스트를 나누어 순차 요청하고 반복 텍스트는 재사용합니다. 인라인 서식과 분할 경계에서는 번역 문맥이 어색할 수 있습니다. 번역 진행률과 취소 버튼을 제공하며, 취소하거나 다른 파일을 열면 이후 요청과 결과 적용을 중단합니다. 이미 전송된 요청은 완료될 수 있습니다. 오류가 발생하면 원문이 유지됩니다.

이 기능은 공식 Google Cloud Translation API가 아닌 **비공식 웹 번역 연결**입니다. 서비스 변경이나 요청 제한으로 중단될 수 있으며, 자동 재시도나 제한 우회는 하지 않습니다. 번역 결과는 다른 문서를 열거나 다시 읽을 때 지워집니다. 웹 미리보기에서는 Google 번역 버튼을 사용할 수 없습니다.

### 사용 중인 AI 서비스로 번역

**AI로 번역**을 누르면 원본 Markdown과 번역 요청문을 준비합니다. **요청문 복사** 후 **ChatGPT / Gemini / Claude 열기**로 기본 브라우저에서 서비스를 열어 붙여넣고 전송합니다. 다른 AI 서비스에도 같은 요청문을 사용할 수 있습니다. 복사가 지원되지 않는 환경에서는 선택된 요청문을 Ctrl+C / Cmd+C로 복사하세요.

문서가 자동으로 전송되거나 URL에 포함되지 않습니다. 사용자가 해당 서비스에서 직접 전송하며, 번역 결과도 해당 서비스에서 확인합니다. 앱으로 결과를 자동 수신하는 API 연동은 포함하지 않습니다. 이용 가능 범위는 각 서비스 계정에 따르며, 긴 문서는 나누거나 파일로 첨부해야 할 수 있습니다. 이 보조 기능은 웹 미리보기에서도 사용할 수 있습니다.

구현 참고: [googletrans의 비공식 연결 방식과 제한](https://github.com/ssut/py-googletrans).
